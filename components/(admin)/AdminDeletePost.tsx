...
type Props = {
  deleteReplyModalVisible: boolean;
  setDeleteReplyModalVisible: React.Dispatch<React.SetStateAction<boolean>>;
  reportId: string;
  id: string;
};
...
const AdminDeletePost = ({
  deleteReplyModalVisible,
  setDeleteReplyModalVisible,
  id,
  reportId,
}: Props) => {
  ...
  const handleDelete = async () => {
    ...
    console.log(`postid: ${id} reportId:${reportId}`)
    const querySnapshot = await getDocs(q);

    const deletePromises = querySnapshot.docs.map((document) =>
      deleteDoc(doc(FIRESTORE_DB, "replies", document.id))
    );
